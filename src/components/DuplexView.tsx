...
import { alignedRegion3to5, mismatchColumns } from '../core/align';
...
export default function DuplexView({ result }: { result: GuideResult }) {
  ...
  const highlight = new Set(mismatchColumns(a.guide.length, a.mismatches));

  const guideCells = guide.split('').map((b, i) => (
    <span key={i} className={`bp ${highlight.has(i) ? 'mm' : ''}`}>{b}</span>
  ));
